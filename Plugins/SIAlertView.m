...
+ (void)initialize
{
   ...
   // appearance.messageFont = [UIFont systemFontOfSize:16];
    appearance.messageFont =[UIFont fontWithName:@"Helvetica" size:15.0];
   // appearance.buttonFont = [UIFont systemFontOfSize:[UIFont buttonFontSize]];
    appearance.buttonFont = [UIFont fontWithName:HELVETICABOLD size:14.0];
...
+ (void)showBackground
{
    if (!__si_alert_background_window) {
       ...
       // __si_alert_background_window.alpha = 0;
        __si_alert_background_window.alpha = 0.6;
...
- (void)validateLayout
{
    ...
    if (self.messageLabel) {
        ...
        self.messageLabel.frame = CGRectMake(CONTENT_PADDING_LEFT, y-6, self.containerView.bounds.size.width - CONTENT_PADDING_LEFT * 2, height); //y-6 is placed in place of y
        y += height;
    }
...
{
	if (self.title) {
        UIImageView *dot =[[UIImageView alloc] initWithFrame:CGRectMake(125,-15,50,50)];
        dot.image=[UIImage imageNamed:@"cashBackinfo.png"];
        [self.containerView addSubview:dot];
		...
		if (!self.titleLabel) {
          ...
          // self.titleLabel.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"cashBackinfo.png"]];
#if __IPHONE_OS_VERSION_MIN_REQUIRED >= __IPHONE_6_0
...
{
    /*Latter added function*/
    CGRect rect = CGRectMake(0.0f, 0.0f, 65.0f, 49.0f);
...
- (UIButton *)buttonForItemIndex:(NSUInteger)index
{
    ...
    normalImage = [SIAlertView imageWithColor:ThemeColor];
    highlightedImage = [SIAlertView imageWithColor:ThemeColor];
